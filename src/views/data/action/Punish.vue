    ...
    <div class="content">
        <div class="querybar">
            <a-form layout="inline" :form="condition">
                ...
                <a-form-item>
                    <a-button type="primary" @click="queryXZCFXW">查询</a-button>
                </a-form-item>
                <a-form-item>
        ...
        >
            <xzcfForm v-if="visible && types != 'detail'" :item="selectedItem" :types="types" />
            <xzcfdetail v-if="visible && types == 'detail'" :types="types" :item="selectedItem" />
        </a-modal>
...
import xzcfForm from "../../../components/action/Punish/xzcfForm.vue";
import xzcfdetail from "../../../components/action/Punish/xzcfdetail.vue";
...
export default {
    ...
    methods: {
        ...
        // 组件内触发关闭弹窗
        closeModel() {
            this.$root.$on("updataCLosexzcf", (msg) => {
                this.handleOk();
            });
        },
        // 删除弹窗
        showDeleteConfirm(e) {
            // console.log(e);
            let id = e.id;
            let deletelist = this.deleteXZCFXW;
            this.$confirm({
                title: "你确定要删除这条信息吗?",
        ...
        queryXZCFXW() {
            ...
            var _this = this;
            this.axios
                .post("action/punish/query", this.condition)
                .then((res) => {
                    // console.log(res);
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                for (let i = 0; i < len; i++) {
                    _this.importData[i]["行政相对人"] =
                        _this.importData[i]["行政相对人（单位/个人）"];
                    _this.importData[i]["行政相对人编码"] =
                        _this.importData[i]["行政相对人(单位/个人)编码"];
                    _this.importData[i]["监管行为监管对象类型"] =
                        _this.importData[i]["监管对象类型"];
                    _this.importData[i]["移送时间"] =
                        _this.importData[i]["移送日期"];
                }
                let result = verify.verify(_this.importData, "行政处罚行为");
                console.log(result);
                if (result.pass) {
        ...
        handleUpload: function () {
            this.axios
                .post("action/punish/insert", this.importData)
                .then((res) => {
                    console.log(res);
                    if (res.data.code == 200) {
