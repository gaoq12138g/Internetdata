<template>
    <div class="content">
        <div class="querybar">
            <a-form layout="inline" :form="condition">
                <a-form-item label="事件级别">
                    ...
                    <a-input allow-clear="allow-clear" v-model="condition.事件级别" />
                </a-form-item>
                <a-form-item label="所属行业">
                    <a-input allow-clear="allow-clear" v-model="condition.所属行业" />
                </a-form-item>
                <a-form-item label="紧急类型">
                    <a-input allow-clear="allow-clear" v-model="condition.紧急类型" />
                </a-form-item>
                <a-form-item>
        ...
        <a-modal v-model="modalVisible" :title="title" :body-style="modalStyle" width="1100px" centered :footer="null" :maskClosable="false" :afterClose="queryListData">
            <detailPlanForm :form="selectedItem" v-if="title == '查看'"></detailPlanForm>
            <planForm :item="selectedItem" :type="title" v-if="title == '新增' && modalVisible || title == '修改' && modalVisible"></planForm>
        </a-modal>
        <!-- 上传 -->
...
<script>
import planForm from '@/components/knowledge/Plan/planForm.vue'
import detailPlanForm from '@/components/knowledge/Plan/detailPlanForm.vue'
...
export default {
    ...
    methods: {
        ...
        queryListData() {
            this.axios
                .post("knowledge/plan/query", this.condition)
                .then((res) => {
                    console.log(res);
        ...
        // 新增关闭弹窗
        closeModalVisible() {
            this.$root.$on("closeModalVisiblePlan", (msg) => {
                // console.log(msg);
                // this.forceList = msg;
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                let result = verify.verify(_this.importData, "预案库");
        ...
        handleUpload() {
            ...
            this.axios
                .post("knowledge/plan/insert", this.importData)
                .then((res) => {
                    if (res.data.code == 200) {
        ...
        deleteItem(item, index) {
            ...
            this.$confirm({
                ...
                onOk() {
                    _this.axios
                        .get("knowledge/plan/delete/" + item.id)
                        .then((res) => {
                            console.log(res);
                            if (res.data.code == 200) {
                                _this.queryListData();
