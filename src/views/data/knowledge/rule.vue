<template>
    <div  class="content">
         <div class="querybar">
            <a-form layout="inline" :form="condition">
                <a-form-item label="规则名称">
                    <a-input allow-clear="allow-clear" v-model="condition.规则名称" />
                </a-form-item>
                <a-form-item label="领域">
                    <a-input allow-clear="allow-clear" v-model="condition.领域" />
                </a-form-item>
                <a-form-item>
        ...
        <a-modal v-model="modalVisible" :title="title" :body-style="modalStyle" width="1100px" centered :footer="null" :maskClosable="false" :afterClose="queryListData">
            <detailRuleForm :form="selectedItem" v-if="title == '查看'"></detailRuleForm>
            <ruleForm :item="selectedItem" :type="title" v-if="title == '新增' && modalVisible || title == '修改' && modalVisible"></ruleForm>
        </a-modal>
        <!-- 上传 -->
...
<script>
import ruleForm from '../../../components/knowledge/Rule/ruleForm.vue';
import detailRuleForm from '../../../components/knowledge/Rule/detailRuleForm.vue';
...
export default {
    ...
    methods: {
        ...
        queryListData() {
            this.axios
                .post("knowledge/rule/query", this.condition)
                .then((res) => {
                    console.log(res);
        ...
        // 新增关闭弹窗
        closeModalVisible() {
            this.$root.$on("closeModalVisibleRule", (msg) => {
                // console.log(msg);
                // this.forceList = msg;
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                let result = verify.verify(_this.importData, "规则信息库");
        ...
        handleUpload() {
            ...
            this.axios
                .post("knowledge/rule/insert", this.importData)
                .then((res) => {
                    if (res.data.code == 200) {
        ...
        deleteItem(item, index) {
            ...
            this.$confirm({
                ...
                onOk() {
                    _this.axios
                        .get("knowledge/rule/delete/" + item.id)
                        .then((res) => {
                            console.log(res);
                            if (res.data.code == 200) {
                                _this.queryListData();
    ...
    mounted() {
        ...
        // 新增修改的弹窗高度
        this.modalStyle.height = height * 0.6 + "px";
        // 导入的弹窗高度
