<template>
    <div class="content">
        <div class="querybar">
            <a-form layout="inline" :form="condition">
                <a-form-item label="姓名：">
                    <a-input allow-clear="allow-clear" v-model="condition.姓名" />
                </a-form-item>
                <a-form-item label="职级：">
                    <a-select v-model="condition.职级" placeholder="职级" style="width:205px">
                        <a-select-option
                ...
                <a-form-item label="身份证号：">
                    <a-input allow-clear="allow-clear" v-model="condition.身份证号" />
                </a-form-item>
                <a-form-item>
        ...
        >
            <memberForm v-if="visible && types != 'detail'" :item="selectedItem" :types="types" />
            <memberdetail v-if="visible && types == 'detail'" :item="selectedItem" :types="types" />
        </a-modal>
...
export default {
    ...
    methods: {
        ...
        // 组件内触发关闭弹窗
        closeModel() {
            this.$root.$on("updataCLoseOfficer", (msg) => {
                this.handleOk();
            });
        },
        ...
        // 查询列表
        query() {
            this.axios.post("officer/query", this.condition).then((res) => {
                console.log(res);
        ...
        // 删除
        delete(id) {
            this.axios.get("officer/delete/" + id).then((res) => {
                console.log(res);
                if ((res.data.code = 200)) {
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                let result = verify.verify(_this.importData, "执法人员");
                console.log(result);
                let len = _this.importData.length;

                for (let i = 0; i < len; i++) {
                    _this.importData[i].所属主体全称 =
                        _this.importData[i]["所属主体（受委托组织）全称"];
                }
        ...
        handleUpload: function () {
            this.axios.post("officer/insert", this.importData).then((res) => {
                console.log(res);
                if (res.data.code == 200) {
