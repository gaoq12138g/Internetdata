<template>
    <div class="content">
        ...
        <a-modal v-model="modalVisible" :title="title" :body-style="modalStyle" width="1100px" centered :footer="null" :maskClosable="false" :afterClose="queryListData">
            <detailAnnualProgramForm :form="selectedItem" v-if="title == '查看'"></detailAnnualProgramForm>
            <annualProgramForm :item="selectedItem" :type="title" v-if="title == '新增' && modalVisible || title == '修改' && modalVisible"></annualProgramForm>
        </a-modal>
        <!-- 上传 -->
...
<script>
import detailAnnualProgramForm from "@/components/random/annualProgram/detailAnnualProgramForm.vue";
import annualProgramForm from "@/components/random/annualProgram/annualProgramForm.vue";
...
export default {
    ...
    methods: {
        ...
        queryListData() {
            this.axios
                .post("random/annualProgram/query", this.condition)
                .then((res) => {
                    console.log(res);
        ...
        // 新增关闭弹窗
        closeModalVisible() {
            this.$root.$on("closeModalVisibleAnnualProgram", (msg) => {
                // console.log(msg);
                // this.forceList = msg;
        ...
        handleUpload() {
            ...
            this.axios
                .post("random/annualProgram/insert", this.importData)
                .then((res) => {
                    if (res.data.code == 200) {
        ...
        deleteItem(item, index) {
            ...
            this.$confirm({
                ...
                onOk() {
                    _this.axios
                        .get("random/annualProgram/delete/" + item.id)
                        .then((res) => {
                            console.log(res);
                            if (res.data.code == 200) {
                                _this.queryListData();
