    ...
    <div class="content">
        ...
        <div class="dataTable">
            <a-table :columns="columns" :data-source="listData" :rowKey="(record,index)=>{return index}" :pagination="pagination" :rowClassName="rowClassName" :scroll="{y:800}">
                <span slot="jgbm" slot-scope="jgbm">{{ jgbm | dictionary('江苏省监管部门代码') }}</span>
                <span slot="ccbl" slot-scope="ccbl">{{ ccbl | addPercent() }}</span>
        ...
        <a-modal v-model="modalVisible" :title="title" :body-style="modalStyle" width="1100px" centered :footer="null" :maskClosable="false" :afterClose="queryListData">
            <detailSpecialProgramForm :form="selectedItem" v-if="title == '查看'"></detailSpecialProgramForm>
            <specialProgramForm :item="selectedItem" :type="title" v-if="title == '新增' && modalVisible || title == '修改' && modalVisible"></specialProgramForm>
        </a-modal>
        <!-- 上传 -->
...
<script>
import detailSpecialProgramForm from "@/components/random/specialProgram/detailSpecialProgramForm.vue";
import specialProgramForm from "@/components/random/specialProgram/specialProgramForm.vue";
...
export default {
    ...
    methods: {
        ...
        queryListData() {
            this.axios
                .post("random/specialProgram/query", this.condition)
                .then((res) => {
                    console.log(res);
        ...
        // 新增关闭弹窗
        closeModalVisible() {
            this.$root.$on("closeModalVisibleSpecialProgram", (msg) => {
                // console.log(msg);
                // this.forceList = msg;
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                let result = verify.verify(_this.importData, "双随机计划");
        ...
        handleUpload() {
            ...
            this.axios
                .post("random/specialProgram/insert", this.importData)
                .then((res) => {
                    if (res.data.code == 200) {
        ...
        deleteItem(item, index) {
            ...
            this.$confirm({
                ...
                onOk() {
                    _this.axios
                        .get("random/specialProgram/delete/" + item.id)
                        .then((res) => {
                            console.log(res);
                            if (res.data.code == 200) {
                                _this.queryListData();
    ...
    mounted() {
        ...
        // 新增修改的弹窗高度
        this.modalStyle.height = height * 0.55 + "px";
        // 导入的弹窗高度
