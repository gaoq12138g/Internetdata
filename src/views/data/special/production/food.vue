<template>
    <div class="content">
        ...
        >
            <foodForm v-if="visible && types != 'detail'" :item="selectedItem" :types="types" />
            <fooddetail v-if="visible && types == 'detail'" :item="selectedItem" :types="types" />
        </a-modal>
...
import foodForm from "@/components/special/food/foodForm";
import fooddetail from "@/components/special/food/fooddetail";
...
export default {
    ...
    methods: {
        ...
        // 组件内触发关闭弹窗
        closeModel() {
            this.$root.$on("updataCLosefood", (msg) => {
                this.handleOk();
            });
        },
        ...
        query() {
            this.axios
                .post("special/food/query", this.condition)
                .then((res) => {
                    console.log(res);
        ...
        // 删除
        delete(id) {
            this.axios.get("special/food/delete/" + id).then((res) => {
                console.log(res);
                if ((res.data.code = 200)) {
        ...
        handleChange: function (a) {
            ...
            importExcel(a.file).then((data) => {
                ...
                let result = verify.verify(_this.importData, "食品");
                console.log(result);
        ...
        handleUpload: function () {
            this.axios
                .post("special/food/insert", this.importData)
                .then((res) => {
                    console.log(res);
                    if (res.data.code == 200) {
    ...
    mounted() {
        ...
        this.modalStyle.height = height * 0.67 + "px";
